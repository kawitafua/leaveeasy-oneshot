...
**แก้เพิ่ม (หลังทดสอบพบว่าขาด):** `leave-request-detail.html` + `js/leave-request-detail.js` ตอนนี้มี UI ให้ **hr** "กำหนดผู้อนุมัติให้แต่ละใบ" ตามสิทธิ์ในตารางหัวข้อ 2 แล้ว (dropdown เลือกจากผู้ใช้ role=manager + ปุ่ม "กำหนดผู้อนุมัติ" แสดงเฉพาะตอน role เป็น hr และใบยังสถานะ `รอพิจารณา`) — ก่อนหน้านี้ Security Rules เปิดทางไว้แล้วแต่ไม่มีหน้าจอให้ใช้จริง ใบลาจึงค้างขึ้น "ยังไม่ได้กำหนดผู้อนุมัติ" ตลอด ตอนนี้แก้แล้ว
