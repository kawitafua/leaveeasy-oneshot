...
ต้องล็อกอินก่อน(function (user) {
  var กล่องเนื้อหา = document.getElementById("เนื้อหาเฉพาะhr");

  // ── สัปดาห์ที่ 8: หน้านี้ใช้ได้เฉพาะ hr เท่านั้น (สเปคหัวข้อ 4 หน้าที่ 4) ──
  //    ต้องเช็ก role ก่อนวาดฟอร์ม/ตาราง ไม่งั้นคนอื่นจะเห็นฟอร์มแล้วกดพังทีหลัง
  //    เพราะ Security Rules ปฏิเสธ create/update/delete ของ leaveTypes ไปแล้วสำหรับคนที่ไม่ใช่ hr
  db.collection("users").doc(user.uid).get().then(function (snap) {
    var role = snap.exists ? snap.data().role : "employee";
    if (role !== "hr") {
      กล่องเนื้อหา.innerHTML =
        '<div class="card"><p>⛔ คุณไม่มีสิทธิ์เข้าถึงหน้านี้ — หน้าจัดการประเภทการลาใช้ได้เฉพาะฝ่ายบุคคล (hr) เท่านั้น</p></div>';
      return;
    }
    เริ่มหน้าจัดการประเภท();
  }).catch(function (err) {
    กล่องเนื้อหา.innerHTML = "<p>อ่านข้อมูลผู้ใช้ไม่สำเร็จ: " + esc(err.message) + "</p>";
  });

  function เริ่มหน้าจัดการประเภท() {
